...
if(isset($_GET['destaque'])){
	...
	if(($_GET['destaque'])==1) {
		if(isset($_GET['cod_anuncioaband'])) { 
			$cod_anuncioabandpettsi=$_GET['cod_anuncioaband'];
			$queryativo="update anuncioaband set destaque=1 where cod_anuncioaband='".$_GET['cod_anuncioaband']."'";	
	...
	if(($_GET['destaque'])==0) {
		if(isset($_GET['cod_anuncioaband'])) {
			$cod_anuncioaband=$_GET['cod_anuncioaband'];
			$queryativo="update anuncioaband set destaque=0 where cod_anuncioaband='".$_GET['cod_anuncioaband']."'";	
			$resultativo=mysqli_query($ligax,$queryativo);
		}
	}
...
<section id="page-breadcrumb">
		...
		<?php
		$consulta="select * from anuncioaband order by $campo $ordem";
		...
		if ($result) {
			...
			<?php
				...
				$contacto_anuncioaband=$registo['contacto_anuncio'];
				...
				<td class='td_lista' align='center'><a href=index.php?pagina=sabermaisaband&cod_anuncioaband=$cod_anuncioaband>".$cod_anuncioaband."</a></td>
				<td class='td_lista' align='center'>".$data_post."</td>
				<td class='td_lista' align='center'>".$email_utilizador."</td>
				<td class='td_lista' align='center'>".$contacto_anuncioaband."</td>
				<td class='td_lista' align='center'>".$destaque."</td> 
				<td class='td_lista' colspan='2' align='center'><a href='index.php?pagina=listar_destaqueaband&cod_anuncioaband=$cod_anuncioaband#popup1' class='button primary fit'>Mudar</a></td>
				"); ?>
			<input type="hidden" name="cod_anuncioaband" value="<?php echo $cod_anuncioaband; ?>"/>
			<div id="popup1" class="overlay">
				...
				<p>Aviso! A decis&atilde;o que tomar pode ser alterada!</p>
					<div class="content">
						<center><a href="index.php?pagina=listar_destaqueaband&destaque=1&cod_anuncioaband=<?php echo $_GET['cod_anuncioaband']; ?>" class="button primary">Colocar nos Destaques</a>
						<br><br><a href="index.php?pagina=listar_destaqueaband&destaque=0&cod_anuncioaband=<?php echo $_GET['cod_anuncioaband']; ?>" class="button primary">Retirar dos Destaques</a></center>
	...
	<?php 
		...
		if ($result) {
			...
			<?php
				...
				<td class='td_lista' align='center'><a href="index.php?pagina=sabermaisaband&cod_anuncioaband=<?php echo $cod_anuncioaband; ?>"><?php echo $cod_anuncioaband; ?></a></td>
				<td class='td_lista' align='center'><?php echo $data_post; ?></td>
				<td class='td_lista' align='center'><?php echo $email_utilizador; ?></td>
				<td class='td_lista' align='center'><?php echo $contacto_anuncioaband; ?></td>
				...
				<td class='td_lista' colspan='2' align='center'><a href='index.php?pagina=listar_destaqueaband&cod_anuncioaband=<?php echo $cod_anuncioaband; ?>#popup1' class='button primary fit'>Mudar</a></td>
			...
			echo "<p> p&aacuteg. ";
			if(($pag_ant)&&($pag>1)){
				echo "<a href=\"index.php?pagina=listar_destaqueaband&pag=$pag_ant&campo=$campo&ordem=$ordem\">Anterior</a> |";
			}
			for ($i=1;$i<=$num_pag;$i++) {
				if($i!=$pag) {
					echo " <a href=\"index.php?pagina=listar_destaqueaband&pag=$i&campo=$campo&ordem=$ordem\">$i</a> |";
				} else {
					echo "$i | ";
				}	
			}	
			if ($pag+1<=$num_pag) {
				echo " <a href=\"index.php?pagina=listar_destaqueaband&pag=$pag_seg&campo=$campo&ordem=$ordem\">Seguinte</a>";
			}
			echo "</td></tr>";
		}
		echo "</table>";?>
