...
if(isset($_GET['destaque'])){
	...
	if(($_GET['destaque'])==1) {
		if(isset($_GET['cod_anunciopetsi'])) { 
			$cod_anunciopetsipettsi=$_GET['cod_anunciopetsi'];
			$queryativo="update anunciopetsi set destaque=1 where cod_anunciopetsi='".$_GET['cod_anunciopetsi']."'";	
	...
	if(($_GET['destaque'])==0) {
		if(isset($_GET['cod_anunciopetsi'])) {
			$cod_anunciopetsi=$_GET['cod_anunciopetsi'];
			$queryativo="update anunciopetsi set destaque=0 where cod_anunciopetsi='".$_GET['cod_anunciopetsi']."'";	
			$resultativo=mysqli_query($ligax,$queryativo);
		}
	}
...
<section id="page-breadcrumb">
		...
		<header class="major">
			<h2>An&uacute;ncios Pet-Sitting</h2>
		</header>
		...
		<?php
		$consulta="select * from anunciopetsi order by $campo $ordem";
		...
		if ($result) {
			...
			<?php
				...
				$contacto_anunciopetsi=$registo['contacto_anuncio'];
				...
				<td class='td_lista' align='center'><a href=index.php?pagina=sabermaispetsi&cod_anunciopetsi=$cod_anunciopetsi>".$cod_anunciopetsi."</a></td>
				<td class='td_lista' align='center'>".$data_post."</td>
				<td class='td_lista' align='center'>".$email_utilizador."</td>
				<td class='td_lista' align='center'>".$contacto_anunciopetsi."</td>
				<td class='td_lista' align='center'>".$destaque."</td> 
				<td class='td_lista' colspan='2' align='center'><a href='index.php?pagina=listar_destaquepetsi&cod_anunciopetsi=$cod_anunciopetsi#popup1' class='button primary fit'>Mudar</a></td>
				"); ?>
			<input type="hidden" name="cod_anunciopetsi" value="<?php echo $cod_anunciopetsi; ?>"/>
			<div id="popup1" class="overlay">
				...
				<p>Aviso! A decis&atilde;o que tomar pode ser alterada!</p>
					<div class="content">
						<center><a href="index.php?pagina=listar_destaquepetsi&destaque=1&cod_anunciopetsi=<?php echo $_GET['cod_anunciopetsi']; ?>" class="button primary">Colocar nos Destaques</a>
						<br><br><a href="index.php?pagina=listar_destaquepetsi&destaque=0&cod_anunciopetsi=<?php echo $_GET['cod_anunciopetsi']; ?>" class="button primary">Retirar dos Destaques</a></center>
	...
	<?php 
		...
		if ($result) {
			...
			<?php
				...
				<td class='td_lista' align='center'><a href="index.php?pagina=sabermaispetsi&cod_anunciopetsi=<?php echo $cod_anunciopetsi; ?>"><?php echo $cod_anunciopetsi; ?></a></td>
				<td class='td_lista' align='center'><?php echo $data_post; ?></td>
				<td class='td_lista' align='center'><?php echo $email_utilizador; ?></td>
				<td class='td_lista' align='center'><?php echo $contacto_anunciopetsi; ?></td>
				...
				<td class='td_lista' colspan='2' align='center'><a href='index.php?pagina=listar_destaquepetsi&cod_anunciopetsi=<?php echo $cod_anunciopetsi; ?>#popup1' class='button primary fit'>Mudar</a></td>
				
			<input type="hidden" name="cod_anunciopetsi" value="<?php echo $cod_anunciopetsi; ?>"/>
			<div id="popup1" class="overlay">
				<div class="popup">
				<h2>Tem a certeza que quer mudar o estado do An&uacute;ncio?</h2>
				<a class="close" href="index.php?pagina=listar_destaquepetsi">&times;</a>
				<p>Aviso! A decis&atilde;o que tomar pode ser alterada!</p>
					<div class="content">
						<center><a href="index.php?pagina=listar_destaquepetsi&destaque=1&cod_anunciopetsi=<?php echo $_GET['cod_anunciopetsi']; ?>" class="button primary">Colocar nos Destaques</a>
						<br><br><a href="index.php?pagina=listar_destaquepetsi&destaque=0&cod_anunciopetsi=<?php echo $_GET['cod_anunciopetsi']; ?>" class="button primary">Retirar dos Destaques</a></center>
			...
			echo "<p> p&aacuteg. ";
			if(($pag_ant)&&($pag>1)){
				echo "<a href=\"index.php?pagina=listar_destaquepetsi&pag=$pag_ant&campo=$campo&ordem=$ordem\">Anterior</a> |";
			}
			for ($i=1;$i<=$num_pag;$i++) {
				if($i!=$pag) {
					echo " <a href=\"index.php?pagina=listar_destaquepetsi&pag=$i&campo=$campo&ordem=$ordem\">$i</a> |";
				} else {
					echo "$i | ";
				}	
			}	
			if ($pag+1<=$num_pag) {
				echo " <a href=\"index.php?pagina=listar_destaquepetsi&pag=$pag_seg&campo=$campo&ordem=$ordem\">Seguinte</a>";
			}
			echo "</td></tr>";
		}
		echo "</table>";?>
